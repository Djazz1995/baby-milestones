...
**S:** `loading` skeleton cards · `empty` warm panel + CTA (owner) / "No moments yet" (viewer) — **born**: "Start {name}'s story"; **pregnancy**: countdown card + "Start the story early" (capture the bump / a scan / the first kick), encouraging a pre-birth moment · `error` retry · `offline` cached + banner · pregnancy vs born header.
**FAB vs empty CTA (mutually exclusive):** when the timeline **has moments**, show the **FAB** (no CTA). When it's **empty**, show only the empty-state **CTA button** (no FAB — never both; two add-buttons on one screen reads as clutter). Applies to born + pregnancy empties.
**Pregnancy → born entry:** the pregnancy timeline shows a "Baby has arrived? · Record the birth" shortcut below the header (owner; surfaced as the due date nears), and the Baby profile has the primary "Baby has arrived" button — both open the record-birth sheet (FLOWS §4).
**role:** viewer = no FAB, no empty CTA.

## 7. Add moment  ⭐ core
**No type picker** — one sheet; the moment's `type` is **derived from what you add**, not chosen. **R:** **media strip** — multi-select photos + videos (mixed; drag to reorder, per-item remove, add-more tile, video badge, first = cover) · **body/caption** field ("Write something… (or just a caption)") · **"Add a voice note"** row (optional spoken caption; record → waveform + timer → attached play/delete) · **"Add measurements"** row (tap → **expands inline** into weight + length number fields, same pattern as the voice-note row; optional → feeds the growth chart; empty = no measurement) · **attach milestone** (optional) · **date** confirm (default now) · primary "Post".
