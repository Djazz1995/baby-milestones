...
| Component | Spec |
|---|---|
| **AgeIndicator** | The signature element. `display` serif, `textPrimary`. Pregnancy: "22 weeks pregnant" + soft teal progress ring around avatar. Born: "8 months old". Sits in Timeline header (= **today's** age) + Baby profile. Derived via `AgeService.compute(baby, today)`. |
